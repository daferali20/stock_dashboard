        ...
        try:
            ...
            if not df.empty and 'Close' in df.columns:
                ...
                if not close_series.empty:
                    ...
                    with col1:
                        st.metric(label=name, value=f"{latest_value:,.2f}", delta=delta_pct)
                    with col2:
            ...
            if not data.empty:
                ...
                with st.spinner('جاري تحليل البيانات...'):
                    try:
                        ...
                        if model:
                            ...
                            col2.metric("التنبؤ للغد", f"{pred_price:.2f}", delta=f"{change_pct:.2f}%", delta_color="inverse" if change_pct < 0 else "normal")
                            st.info("""
                            **تفسير النتائج:**
                ...
                try:
                    ...
                    if not sp500.empty and 'Close' in sp500.columns:
                        ...
                        fig = go.Figure()
                        fig.add_trace(go.Scatter(x=norm_data.index, y=norm_data, name=ticker, line=dict(color='royalblue', width=2)))
                        fig.add_trace(go.Scatter(x=norm_sp500.index, y=norm_sp500, name="S&P 500", line=dict(color='gray', width=2)))
                        fig.update_layout(title="أداء السهم مقارنة بمؤشر S&P 500", yaxis_title="النسبة المئوية للتغير", hovermode="x unified")
                ...
                try:
                    ...
                    if recs is not None and not recs.empty:
                        st.dataframe(recs.style.highlight_max(subset=['to grade'], color='lightgreen').set_properties(**{'text-align': 'right'}).format({'to grade': '{:.1f}'}))
                    else:
